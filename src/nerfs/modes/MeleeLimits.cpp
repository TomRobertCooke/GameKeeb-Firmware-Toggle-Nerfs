...
#include "nerfs/modes/Melee20Button.hpp"
...
#define TRAVELTIME_EASY3 \
    8 // ms for 112+cubic it takes 83% to get to dash, for 80+linear it takes 80% to get to dash
...
#define TIMELIMIT_DOWNUP \
    (16 * 3 * 250) // units of 4us; how long after a crouch to upward input should it begin a jump?
#define JUMP_TIME \
    (16 * 2 * 250 \
    ) // units of 4us; after a recent crouch to upward input, always hold full up for 2 frames
...
// not used #define TIMELIMIT_DASH 60000//(16*15*250)//units of 4us; last dash time prior to a pivot
// input; 15 frames
...
// for pivot nerfs, we want to record only movement between dash zones, ignoring movement within
// zones
typedef struct {
    uint16_t timestamp; // in samples
    uint8_t zone;
...
void getIOStates(const RectangleInput &rectangleInput, InputState &inputState, OutputState &outputState) {
    inputState.left = rectangleInput.left;
    inputState.right = rectangleInput.right;
    inputState.down = rectangleInput.down;
    inputState.up = rectangleInput.up;
    inputState.c_left = rectangleInput.cLeft;
    inputState.c_right = rectangleInput.cRight;
    inputState.c_down = rectangleInput.cDown;
    inputState.c_up = rectangleInput.cUp;
    inputState.a = rectangleInput.a;
    inputState.b = rectangleInput.b;
    inputState.x = rectangleInput.x;
    inputState.y = rectangleInput.y;
    inputState.l = rectangleInput.l;
    inputState.r = rectangleInput.r;
    inputState.z = rectangleInput.z;
    inputState.lightshield = rectangleInput.ls;
    inputState.midshield = rectangleInput.ms;
    inputState.start = rectangleInput.start;
    inputState.mod_x = rectangleInput.mx;
    inputState.mod_y = rectangleInput.my;

    Melee20ButtonOptions melee20ButtonOptions;
    melee20ButtonOptions.crouch_walk_os = true;
    melee20ButtonOptions.teleport_coords = false;
    Melee20Button melee20Button(socd::SOCD_NEUTRAL, melee20ButtonOptions);
    melee20Button.UpdateOutputs(inputState, outputState);
}

void makeNerfedReport(const OutputState &nerfedOutputs, gc_report_t *gcReport) {
    //Digital Outputs
    gcReport->a = nerfedOutputs.a;
    gcReport->b = nerfedOutputs.b;
    gcReport->x = nerfedOutputs.x;
    gcReport->y = nerfedOutputs.y;
    gcReport->z = nerfedOutputs.buttonR;
    gcReport->l = nerfedOutputs.triggerLDigital;
    gcReport->r = nerfedOutputs.triggerRDigital;
    gcReport->start = nerfedOutputs.start;
    gcReport->dpad_left = nerfedOutputs.dpadLeft | nerfedOutputs.select;
    gcReport->dpad_right = nerfedOutputs.dpadRight | nerfedOutputs.home;
    gcReport->dpad_down = nerfedOutputs.dpadDown;
    gcReport->dpad_up = nerfedOutputs.dpadUp;

    // Analog outputs
    gcReport->stick_x = nerfedOutputs.leftStickX;
    gcReport->stick_y = nerfedOutputs.leftStickY;
    gcReport->cstick_x = nerfedOutputs.rightStickX;
    gcReport->cstick_y = nerfedOutputs.rightStickY;
    gcReport->l_analog = nerfedOutputs.triggerLAnalog;
    gcReport->r_analog = nerfedOutputs.triggerRAnalog;
}

uint8_t isEasy(const uint8_t x, const uint8_t y) {
    // is it on the rim?
    const uint8_t xnorm =
        (x > ANALOG_STICK_NEUTRAL ? (x - ANALOG_STICK_NEUTRAL) : (ANALOG_STICK_NEUTRAL - x));
    const uint8_t ynorm =
        (y > ANALOG_STICK_NEUTRAL ? (y - ANALOG_STICK_NEUTRAL) : (ANALOG_STICK_NEUTRAL - y));
...
uint8_t lookback(const uint8_t currentIndex, const uint8_t samplesBack) {
...
uint8_t isTapSDI(
    const sdizonestate zoneHistory[HISTORYLEN],
    ...
    if (zoneList[0] != zoneList[1] && (zoneList[0] == zoneList[2]) &&
        (zoneList[1] == zoneList[3])) {
        // check the time duration
        const uint16_t timeDiff0 =
            (currentTime - timeList[2]) *
            sampleSpacing; // make sure things aren't reliant on long-past inputs
        const uint16_t timeDiff1 =
            (timeList[0] - timeList[2]) *
            sampleSpacing; // rising edge to rising edge, or falling edge to falling edge
        // const uint16_t timeDiff2 = (timeList[0] - timeList[1])*sampleSpacing;//rising to falling,
        // or falling to rising We want to nerf it if there is more than one press every 6 frames,
        // but not if the previous press or release duration is less than 1 frame
        if (!staleList[2] && (timeDiff0 < TIMELIMIT_TAP_PLUS && timeDiff1 < TIMELIMIT_TAP &&
                              timeDiff0 > TIMELIMIT_DEBOUNCE)) {
            if ((zoneList[0] == 0) ||
                (zoneList[1] == 0)) { // if one of the pairs of zones is zero, it's tapping a
                                      // cardinal (or tapping a diagonal modifier)
    ...
    for (int i = 0; i < historyLength; i++) {
        ...
        if (popcnt == 0) {
            ...
            diagCount++;
            diagZone = diagZone &
                       zoneList[i]; // if two of these don't match, it'll have zero or one bits set
        }
    }
    { // to limit scope of these vars
        ...
        // const bool recentOrig = (zoneList[1] & zoneList[2]) == 0;//may be too lenient in case
        // people throw in modifier taps? check whether the input was fast enough
    ...
    for (int i = 0; i < historyLength; i++) {
        ...
        if (popcnt == 0) {
            ...
            cardCount++;
            cardZone =
                cardZone & zoneList[i]; // if there are two different cardinals then it won't count
        } else {
            diagCount++;
            diagZone = diagZone &
                       zoneList[i]; // if two of these don't match, it'll have zero or one bits set
    ...
    { // to limit scope of these vars
        ...
        const bool shortTime =
            ((timeList[0] - timeList[3]) * sampleSpacing < TIMELIMIT_WANK) && !staleList[3];
    ...
    // return the last cardinal in the zone list before the last diagonal, useful for SDI diagonal
    // nerfs.
    bool lookNow = false;
...
void travelTimeCalc(
    ...
    if (type == T_Lin) {
        const uint16_t travelTimeElapsed =
            timeElapsed / msTravel; // 250 times the fraction of the travel time elapsed
        ...
        const uint16_t cappedTT = min((uint16_t)256, travelTimeElapsed);
        ...
        //         doneTraveling = true;
        //     }
        //     const Fixed88 timeElapsedPercent = fastDiv(intToFixed(int8_t(timeElapsed>>6)),
        //     intToFixed(int8_t(usTravel4>>6)));
        //
        //     Fixed88 interpolatedTime = max(min(timeElapsedPercent, intToFixed(int8_t(1))),
        //     intToFixed(int8_t(0)));
        //
        //     /*if(type == T_Quad) {
        //      * */
...
void limitOutputs(
    const uint16_t sampleSpacing, // in units of 4us
    ...
    //   an empty pivot has likely occurred (0.5 to 1.5 frames?) ============ this depends on our
    //   travel time implementation unless we store the analog value history too
    //     how?
    ...
    // we want to continue the previous travel time, but if it's an airdodge with a prohibited angle
    // destination, we overwrite the angle. this way the travel itself never skips straight to the
    // destination.
    ...
    if (inputs.l || inputs.r) {
        ...
        // the trigger was the only input that changed, potentially causing a retargeting without
        // restarting the travel time
        ...
        const uint16_t xInMag =
            xIn > ANALOG_STICK_NEUTRAL ? xIn - ANALOG_STICK_NEUTRAL : ANALOG_STICK_NEUTRAL - xIn;
        const uint16_t yInMag =
            yIn > ANALOG_STICK_NEUTRAL ? yIn - ANALOG_STICK_NEUTRAL : ANALOG_STICK_NEUTRAL - yIn;
        ...
        // if the coordinates changed
        if (aHistory[currentIndexA].x != rawOutputIn.leftStickX ||
            ...
            if (((yInMag * 157 < xInMag * 80) || (xInMag * 157 < yInMag * 80)) && xInMag &&
                yInMag) { // 157 and 80 are the closest ratio to 27 degrees (27.0013)
                // the closest coordinate inside the melee unit circle to 27 degrees is 51 and 26
                // (27.0127)
                ...
                if (!(prevInputs.l || prevInputs.r)) {
                    // check if the L/R press coincides with a change in coordinate, indicating L/R
                    // NDM
                    if (prevInputs.left == inputs.left && prevInputs.right == inputs.right &&
                        prevInputs.down == inputs.down && prevInputs.up == inputs.up &&
                        prevInputs.c_left == inputs.c_left &&
                        prevInputs.c_right == inputs.c_right &&
                        prevInputs.c_down == inputs.c_down && prevInputs.c_up == inputs.c_up &&
                        prevInputs.b == inputs.b && prevInputs.lightshield == inputs.lightshield &&
                        prevInputs.midshield == inputs.midshield &&
                        prevInputs.mod_x == inputs.mod_x && prevInputs.mod_y == inputs.mod_y) {
                        ...
                        aHistory[currentIndexA].x = rawOutputIn.leftStickX;
                        aHistory[currentIndexA].y = rawOutputIn.leftStickY;
                        // mark this change as having been skipped so we don't restart travel time
                        // later
                        wavedashSkip = true;
                    }
                }
        ...
        // nerf it
        if (((yInMag * 157 < xInMag * 80) || (xInMag * 157 < yInMag * 80)) && xInMag && yInMag &&
            !wavedashWasNerfed) { // 157 and 80 are the closest ratio to 27 degrees (27.0013)
            // the closest coordinate inside the melee unit circle to 27 degrees is 51 and 26
            // (27.0127)
    ...
    if (tapSDI & BITS_SDI_TAP_CARD) {
        aHistory[currentIndexA].tt = max(aHistory[currentIndexA].tt, (uint8_t)TRAVELTIME_SLOW);
        delayType = T_Lin;
    }
    ...
    for (int i = 0; i < HISTORYLEN; i++) {
        if ((currentTime - pivotZoneHist[i].timestamp) * (sampleSpacing >> 1) >
            15 * 16 * 125) { // 15 frames
            pivotZoneHist[i].stale = true;
        }
    }
    ...
    // current time minus start time of neutral should be used to limit how long the nerf applies
    // for
    pivotdir direction = P_None;
    if (pivotZoneHist[0].zone == 0) {
        if (pivotZoneHist[1].zone == ZONE_L &&
            (pivotZoneHist[2].zone == ZONE_R || pivotZoneHist[3].zone == ZONE_R)) {
    ...
    uint16_t pivotLength =
        (pivotZoneHist[0].timestamp - pivotZoneHist[1].timestamp) * sampleSpacing;
    ...
    // check for staleness
    if (pivotZoneHist[3].stale || pivotZoneHist[2].stale || pivotZoneHist[1].stale ||
        pivotZoneHist[0].stale) {
    ...
    */
    ...
    if (pivotTilt) {
        ...
        if (upTilt && xCoordAbs > yCoordAbs) {
            // Force all upward angles to a minimum of 45deg to prevent pivot uftilt and ensure tap
            // jump
    ...
    static uint16_t timeSinceCrouch =
        100; // must be < 65535 when multiplied by 500, the longest possible sampleSpacing, but
             // bigger than the (timelimit/250)
    ...
    if (timeSinceJump * sampleSpacing < JUMP_TIME && downUpJumping) {
        prelimAY = 255;
        timeSinceCrouch =
            100; // prevent an extra duration jump if the jump ends before the lockout window
    } else {
        downUpJumping = false;
    }
    ...
    if (tapSDI & (BITS_SDI_TAP_DIAG | BITS_SDI_TAP_CRDG | BITS_SDI_WANK)) {
        if (tapSDI & (ZONE_L | ZONE_R)) {
            ...
            // prevent the cardinal axis from dipping below the sdi threshold by preserving its
    ...
    if (sdiZoneHist[currentIndexSDI].zone !=
        sdiZone(rawOutputIn.leftStickX, rawOutputIn.leftStickY)) {
    ...
    for (int i = 0; i < HISTORYLEN; i++) {
        if (currentTime - sdiZoneHist[i].timestamp >
            8 * 16 * 2) { // 8 frames * 16 ms * max 2 samples
            sdiZoneHist[i].stale = true;
        }
    }

    // if we have a new coordinate, record the new info, the travel time'd locked out stick
    // coordinate, and set travel time
    const uint8_t xIn = rawOutputIn.leftStickX;
    const uint8_t yIn = rawOutputIn.leftStickY;
